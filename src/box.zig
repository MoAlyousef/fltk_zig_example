...
pub const Box = struct {
    ...
    pub fn handle(self: *Box, cb: fn (w: ?*c.Fl_Widget, ev: i32 data: ?*c_void) callconv(.C) i32, data: ?*c_void) void {
        c.Fl_Box_handle(self.inner, cb, data);
    }

    pub fn draw(self: *Box, cb: fn (w: ?*c.Fl_Widget, data: ?*c_void) callconv(.C) void, data: ?*c_void) void {

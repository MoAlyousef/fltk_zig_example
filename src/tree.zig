...
pub const Tree = struct {
    ...
    pub fn handle(self: *Tree, cb: fn (w: ?*c.Fl_Widget, ev: i32 data: ?*c_void) callconv(.C) i32, data: ?*c_void) void {
        c.Fl_Tree_handle(self.inner, cb, data);
    }

    pub fn draw(self: *Tree, cb: fn (w: ?*c.Fl_Widget, data: ?*c_void) callconv(.C) void, data: ?*c_void) void {
